...
export function userDataVerification(user: User): Response | null {

    const errors: Record<string, string> = {};

    if (user.name &&user.name!==null) {
        if (user.name.length < 3) {
            errors[UserFieldNames.NAME] = UserValidationErrorMessages.NAME_LENGTH;
        }
    } else {
    ...
    if (user.phoneNo&&user.phoneNo!==null) {
        if (user.phoneNo.length !==12) {
            errors[UserFieldNames.PHONE_NO] = UserValidationErrorMessages.PHONE_NO_INVALID;
        }
    } else {
        errors[UserFieldNames.PHONE_NO] = UserValidationErrorMessages.PHONE_NO_REQUIRED;
    }
    ...
    if (user.address&&user.address!==null) {
        if (user.address.length < 6) {
            errors[UserFieldNames.ADDRESS] = UserValidationErrorMessages.ADDRESS_INVALID;
        }
    } else {
        errors[UserFieldNames.ADDRESS] = UserValidationErrorMessages.ADDRESS_REQUIRED;
    }
