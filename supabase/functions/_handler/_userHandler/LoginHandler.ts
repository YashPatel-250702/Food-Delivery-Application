...
export async function loginHandler(req:Request):Promise<Response> {
    ...
    try {
      ...
      const {data,error}:{data:User|null,error:PostgrestError|null}=await userLoginRepository(userLogin);
      ...
      if(data===null||data.id===undefined){
        return new Response(JSON.stringify({error:UserErrorMessages.USER_INVALID_CREDENTIALS}),{
            status:401,
            headers:{
                "content-type":"application/json"
            }
